...
const Register = ({ userData, onChange }) => {
  ...
  const registrationHandler = () => {
    if (userData.email && userData.password) {
      setIsLoading(true); //set loading for sign up button
      setTimeout(() => {
  ...
  return (
    <div className=" right-16  top-10    mx-auto flex min-h-[80vh] w-11/12 max-w-md flex-col items-center rounded-md bg-gray-500/30  px-2 py-1 text-center shadow-md shadow-gray-900/80 backdrop-blur-md transition-all duration-200 md:absolute md:w-4/12">
      <h1 className="mt-2 text-xl font-bold text-gray-200">ثبت نام</h1>
      ...
      <Button variant="contained" color="primary" onClick={registrationHandler}>
        ثبت نام
      </Button>
